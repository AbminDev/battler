...
import {buyCard, getShopItems} from "../../endpoints/dungeonEndpoints";
...
export const DungeonShop = ({gold, close, updateGoldAmount, updateSomeStates}: {
  gold: number,
  close: () => void,
  ...
  useEffect(() => {
    ...
    const getCardsForShop = async () => {
      //@ts-ignore
      const getCards = await getShopItems({dungeonId: actualSaves?.dungeonId, stageId: actualSaves?.currentStage, clientId: userId });
      if (getCards.cards.length) {
        // console.log('CARDS FROM SHOP->', getCards.cards);
        getCards.cards.forEach((card, index) => {
          cards.push({
            id: card.cardId,
            lvl: card.stars,
            uid: `${index}`,
...
//setCards([])
  ...
  const removeCardAndBuy = ({cardIndex, cardId, price}:{cardIndex: number, cardId: number, price: number}) => {
    // console.log('removeCardAndBuy');
    const copyCards = [...cards];
    copyCards.splice(cardIndex, 1);
    setCards(copyCards);
    (async () => {
      try {
       const result = await buyCard({stageId: actualSaves?.currentStage!, cardId, clientId: userId});
        updateGoldAmount(gold, price);
        updateSomeStates({gold: result.gold, currentMana: result.currentMana, currentHp: result.currentHp, cardsAtDeck: result.cardsAtDeck});
      } catch (e) {
        console.error(e);
      }
    })();
                  ...
                  <div className="absolute top-[-28px] left-0 right-0 flex justify-self-center">
                    <div className="min-w-[120px] bg-[#847a70] border border-[#18191a] rounded-[2px] p-[1px] mx-auto">
                      <div className="bg-[#351d1e] text-center text-white text-xl p-[7px] leading-none rounded-[1px]
                        border border-[#18191a] shadow-inner-sm-black">{t('shop')}</div>
                    </div>
                  </div>
                  {cards.length ? (
                    <div className="grid grid-cols-3 gap-4 mb-3">
                      {cards.map((card, index) => (
                        <div className="table" key={index}>
                          ...
                          <PopupButton type={gold >= card.cost ? 'gold' : 'gray'} className="w-full"
                             onClick={() => {
                               playSound('button');
                               if (gold >= card.cost) {
                                  removeCardAndBuy({cardId: card.id, cardIndex: index, price: card?.cost})
                               } else {
                                  if (timeoutId) {
                                    setTimeoutId(null);
                                  }
                            ...
                            <div className="flex justify-center items-center text-center gap-1">
                              ...
                              <div
                                className="text-right text-white uppercase leading-[14px] font-[900]">
                                {card?.cost}
                              </div>
                            </div>
                          </PopupButton>
